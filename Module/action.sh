...
if [ -f "$MODDIR/system/usr/keylayout/gpio-keys.kl" ]; then
  TARGET_FILE="$MODDIR/system/usr/keylayout/gpio-keys.kl"
elif [ -f "$MODDIR/system/usr/keylayout/Generic.kl" ]; then
  TARGET_FILE="$MODDIR/system/usr/keylayout/Generic.kl"
else
  TARGET_FILE=""
fi

if [ -z "$TARGET_FILE" ] || [ ! -f "$TARGET_FILE" ]; then
  echo "! Error: Neither gpio-keys.kl nor Generic.kl found in the module!"
  echo "! Checked: $MODDIR/system/usr/keylayout/"
  exit 1
fi
